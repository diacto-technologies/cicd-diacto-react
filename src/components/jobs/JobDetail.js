...
const JobDetail = () => {
  const { domain, authTokens, logoutUser, userDetails } =
  ...
  const fetchJob = async () => {
    ...
    try {
      ...
      alert(error);
      navigate("/app/user/jobs/"); // Redirect to the new route
      setError(error);
      setLoading(false);
    }
  };
        ...
        <>
          <div className=" py-3 h-full overflow-auto">
            ...
            <div className="flex border-y mt-3 justify-center md:justify-start w-full bg-white">
              <Link
                to={`/app/user/jobs/job/${jobId}/overview/`}
                className={` ${
              ...
              <Link
                to={`/app/user/jobs/job/${jobId}/applicants/`}
                className={` ${
                  currentTab.toLowerCase() === "applicants"
                    ? "text-[#3D73FD] border-b-4  border-[#3D73FD] font-semibold"
                    : "text-gray-800 "
                } bg-white    hover:text-sky-700 block px-4 py-2 text-sm `}
              >
              ...
              {/* <Link to={`/app/user/jobs/job/${jobId}/applicant-stages/`} className={` ${currentTab.toLowerCase() === "applicant-stages" ? 'text-[#3D73FD] border-b-4  border-[#3D73FD] font-semibold' : 'text-gray-800 '} bg-white    hover:text-sky-700 block px-4 py-2 text-sm `}>Applicant Stages</Link> */}
