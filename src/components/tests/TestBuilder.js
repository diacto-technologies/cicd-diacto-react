...
const TestBuilder = () => {
  ...
  const { testId,assessmentType } = useParams();
  ...
  async function saveQuestion() {
    ...
    const formattedData = {
      ...
      domain: null,
      choices: JSON.stringify(choices),
      test_id: assessmentType === "test" ? testId : null,
      prebuilt_assessment_id : assessmentType === "prebuilt-assessment" ? testId : null
    };
  ...
  const validateFormData = (data, validateFor) => {
    ...
    if (choices?.length && (selectedAnswerFormat?.value.toLowerCase() === "single" || selectedAnswerFormat?.value.toLowerCase() === "multiple")) {
    ...
    } else if (selectedAnswerFormat?.value.toLowerCase() === "single" || selectedAnswerFormat?.value.toLowerCase() === "multiple") {
  ...
  const handleUpload = async () => {
    ...
    formData.append("assessment_type", assessmentType === "test" ? "user-owned-assessment" : "prebuilt-assessment");
              ...
              <div id="question" className="w-full h-40 rounded-lg p-5">
                ...
                <div className="w-full flex flex-col items-start flex-grow justify-center gap-6 p-8 mt-3 rounded-md border shadow-sm">
                  ...
                  <div className="grid grid-cols-2 w-full gap-5 justify-start">
                    ...
                    {selectedAnswerFormat?.value?.toLowerCase() === "single" &&
                      choices.map((choice) => (
                        <div
                          key={choice.id}
                    ...
                    {selectedAnswerFormat?.value?.toLowerCase() === "multiple" &&
                      choices.map((choice) => (
                        <div
                          key={choice.id}
                    ...
                    {selectedAnswerFormat?.value?.toLowerCase() === "text" && (
                      <label className="text-sm text-gray-400 ">
                    ...
                    {selectedAnswerFormat?.value === "audio" && (
                      <div className="flex items-center justify-start gap-3 w-full">
                        ...
                        <span className=" p-3 rounded-md bg-slate-100 text-sm text-gray-500">
                          {" "}
                          Applicants response will be recorded in audio format
                        </span>
                      </div>
                    )}
                  </div>
